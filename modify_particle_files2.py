...
if(len(sys.argv) != 5):
    sys.exit("The script expects 4 arguments")
else:
...
particle_files = glob.glob(os.path.join(input_dir,"geant4*.txt"))
...
def modify_particle_line(particle_line,delta = 1/10.,mode = 1,ice_model='southpole'):
	...
	new_particle_line = particle_line
        ...
        if particle_id in [1,2,3]:
            ...
            new_particle_line = "{} {:.9e} {:.9e} {:.9e} {:.9e} {:.9e} {:.9e} {:.9e} {}\n".format(particle_id, new_p[0], new_p[1], new_p[2], new_x, new_y, new_z, new_t, new_w)
    elif mode == 3:
        ...
        if (particle_E > 10) and (particle_id in [1,2,3]):
            ...
            new_w = particle_w / delta
            new_particle_line = "{} {:.9e} {:.9e} {:.9e} {:.9e} {:.9e} {:.9e} {:.9e} {}\n".format(particle_id, new_p[0], new_p[1], new_p[2], particle_x, particle_y, particle_z, particle_t, new_w)
    elif mode == 5:
        ## energy cut with particle selection
        if (particle_E > 10) and (particle_id in [1,2,3]):
            new_E = delta* particle_E
            ...
            print("delta Slant:{:.9e} g/cm2 \t delta Length {:.9e} cm delta_t {:.9e}".format(dSlant/(units.g/units.cm**2), dLength/units.cm, delta_t) )
            new_particle_line = "{} {:.9e} {:.9e} {:.9e} {:.9e} {:.9e} {:.9e} {:.9e} {}\n".format(particle_id, new_p[0], new_p[1], new_p[2], vector_1[0], vector_1[1], vector_1[2], new_t, new_w)
    ...
    with open(path2input,'r') as file:
        ...
        for line in lines:
            if line[0] == '#': # header
                ...
                modified_line = modify_particle_line(line,mode=int(mode_),delta=1./float(beta_))
